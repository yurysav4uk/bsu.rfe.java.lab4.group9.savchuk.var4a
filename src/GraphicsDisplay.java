...
// Перо для рисования графика
        graphicsStroke = new BasicStroke(4.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_MITER, 10.0f, new float[]{4, 4, 4, 4, 4, 4, 16, 4, 8, 4, 8, 4}, 0.0f);
...
// Запросить перерисовку компонента, т.е. неявно вызват paintComponent();
        repaint();
    }
...
// Найти минимальное и максимальное значение функции
        for (int i = 1; i < graphicsData.length; i++) {
            if (graphicsData[i][1] < minY) {
                minY = Math.min(graphicsData[i][1], -minY);
            }
...
// Затем (если нужно) отображаются маркеры точек, по которы строился график.
...
// Отобразить график
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point : graphicsData) {
            ...
            star.moveTo(star.getCurrentPoint().getX() - 5, star.getCurrentPoint().getY());
            star.lineTo(star.getCurrentPoint().getX() + 5, center.getY() + 5);
    ...
    protected boolean isnoteven(Double value) {
        var temp = value.intValue();
        if (value % 2 == 1) {
            return true;
        }
        return false;
